...
from atelier.atelier_messages.models import BaseMessage, BaseMessageAttachment


class TestBaseMessage(test.TestCase):
    def test_get_message_class_string(self):
        self.assertEqual(BaseMessage.get_message_class_string(), 'atelier.atelier_messages.models.BaseMessage')

    def test_validate_subject_for_sending_sms(self):
        message = mommy.prepare('atelier_messages.BaseMessage', subject='', message_types=['sms'])
        message.validate_subject_for_sending()
        message2 = mommy.prepare('atelier_messages.BaseMessage', subject='Test', message_types=['sms'])
        message2.validate_subject_for_sending()

    def test_validate_subject_for_sending_email(self):
        message = mommy.prepare('atelier_messages.BaseMessage', subject='', message_types=['email'])
        with self.assertRaises(ValidationError):
            message.validate_subject_for_sending()
        message2 = mommy.prepare('atelier_messages.BaseMessage', subject='Test', message_types=['email'])
        message2.validate_subject_for_sending()

    def test_validate_subject_for_sending_email_and_sms(self):
        message = mommy.prepare('atelier_messages.BaseMessage', subject='', message_types=['email', 'sms'])
        with self.assertRaises(ValidationError):
            message.validate_subject_for_sending()
        message2 = mommy.prepare('atelier_messages.BaseMessage', subject='Test', message_types=['email', 'sms'])
        message2.validate_subject_for_sending()

    def test_validate_message_content_plain_for_sending(self):
        message = mommy.prepare('atelier_messages.BaseMessage', message_content_plain='')
        with self.assertRaises(ValidationError):
            message.validate_message_content_plain_for_sending()
        message2 = mommy.prepare('atelier_messages.BaseMessage', message_content_plain='Test')
        message2.validate_message_content_plain_for_sending()

    def test_clean_message_content_fields_has_html_but_not_plain(self):
        message = mommy.prepare('atelier_messages.BaseMessage',
    ...
    def test_get_email_attachment_ids_sanity(self):
        message = mommy.make('atelier_messages.BaseMessage',
                                message_content_plain='',
                                message_content_html='')
        att1 = mommy.make('atelier_messages.BaseMessageAttachment', message=message)
...
class TestBaseMessageQuerysets(test.TestCase):
    def test_filter_locked_for_edit_status_values(self):
        message_draft = mommy.make('atelier_messages.BaseMessage')
        message_queued_for_prepare = mommy.make(
            'atelier_messages.BaseMessage', status=BaseMessage.STATUS_CHOICES.QUEUED_FOR_PREPARE.value)
        message_preparing = mommy.make(
            'atelier_messages.BaseMessage', status=BaseMessage.STATUS_CHOICES.PREPARING.value)
        message_ready_for_sending = mommy.make(
            'atelier_messages.BaseMessage', status=BaseMessage.STATUS_CHOICES.READY_FOR_SENDING.value)
        message_queued_for_sending = mommy.make(
            'atelier_messages.BaseMessage', status=BaseMessage.STATUS_CHOICES.QUEUED_FOR_SENDING.value)
        message_sending_in_progress = mommy.make(
            'atelier_messages.BaseMessage', status=BaseMessage.STATUS_CHOICES.SENDING_IN_PROGRESS.value)
        message_error = mommy.make(
            'atelier_messages.BaseMessage', status=BaseMessage.STATUS_CHOICES.ERROR.value)
        message_sent = mommy.make(
            'atelier_messages.BaseMessage', status=BaseMessage.STATUS_CHOICES.SENT.value)
...
class TestBaseMessageAttachmentModel(test.TestCase):
    ...
    def test_simple_create_success(self):
        ...
        mommy.make('atelier_messages.BaseMessageAttachment')
    ...
    def test_create_success_with_data(self):
        ...
        attachment = mommy.make(
            'atelier_messages.BaseMessageAttachment',
            message=message,
            attachment_file_url='https://test.com',
            title='title'
        )
    ...
    def test_delete_success(self):
        attachment = mommy.make('atelier_messages.BaseMessageAttachment')
        self.assertEqual(BaseMessageAttachment.objects.count(), 1)
        attachment.delete()
        self.assertEqual(BaseMessageAttachment.objects.count(), 0)
