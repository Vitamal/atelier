...
def send_with_delay(send_kwargs):
    """
    RQ task for sending emails in the background
    :param send_kwargs: kwargs for ``django.core.mail.send_mail``
    """
    from django.core.mail import send_mail as other_sent_mail
    other_sent_mail(**send_kwargs)
...
class AbstractEmail(object):
    ...
    def delay(self):
        """
        Send email as an rq task
        :return: None
        """
        kwargs = self.get_send_mail_kwargs()

        django_rq.enqueue(send_with_delay, kwargs)
